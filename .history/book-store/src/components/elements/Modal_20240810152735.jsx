    ...
    if (content === "book") {
        Object.keys(data).forEach((field) => {
            if (field === "field2" && data[field]) {
                ...
                // Assign title to field2
                initialValues["field2"] = title || "";

                // Assign author to field7 (last field)
                initialValues["field7"] = author || "";
            } else {
                // Shift the remaining fields by one index, starting from field3
                const fieldIndex = parseInt(field.replace("field", ""), 10);
                if (fieldIndex > 2 && fieldIndex < 7) {
                    const newFieldIndex = fieldIndex + 1; // Shift by 1
                    initialValues[`field${newFieldIndex}`] = data[field];
                } else if (fieldIndex === 7) {
                    // If we're at field6 (now moved to field7), it should stay as is (author has already been assigned)
