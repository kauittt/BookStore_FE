...
const Modal = ({ onClose, data, onSave, category, mapping, content }) => {
    ...
    Object.keys(mapping).forEach((field) => {
        if (field === "field0") {
            // Skip field3 initialization as it's handled with field0
        } else if (content === "book" && field === "field2" && data[field]) {
            // Split the field2 value by " - " to get title and author
            const [title, author] = data[field].split(" - ");

            // Assign title to field2
            initialValues[`field${count}`] = title || "";
            count++;

            // Assign author to the next field, which should be field3
            initialValues[`field${count}`] = author || "";
            count++;
        } else {
