...
const Modal = ({ onClose, data, onSave, category, mapping, content }) => {
    ...
    Object.keys(mapping).forEach((field) => {
        // If the content is "book" and the current field is "field2"
        if (content === "book" && field === "field2" && data[field]) {
            // Split the field2 value by " - " to get title and author
            const [title, author] = data[field].split(" - ");
            initialValues["field2"] = title || ""; // Assign title to field2
            initialValues["field3"] = author || ""; // Assign author to field3
        } else if (content === "book" && field === "field3") {
            // Skip field3 initialization as it's handled with field2
        } else {
            // Default behavior
