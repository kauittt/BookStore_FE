...
const Modal = ({ onClose, data, mapping, content, ...props }) => {
    ...
    if (content === "book") {
        Object.keys(mapping).forEach((key) => {
            if (key === "field2" && data[key]) {
                // Split the field2 value by " - " to get name and author
                const [name, author] = data[key].split(" - ");

                // Assign name to field2 and author to field3
                initialValues["field2"] = name || "";
                initialValues["field3"] = author || "";
            } else if (
                key.startsWith("field") &&
                parseInt(key.replace("field", ""), 10) > 2
            ) {
                // Shift the remaining fields by one index
                const fieldIndex = parseInt(key.replace("field", ""), 10) + 1;
                initialValues[`field${fieldIndex}`] = data[key] || "";
            } else {
